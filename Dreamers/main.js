...
(function (Dreamers) {
    ...
    let enemySpawnRecharge = 0;
    //The current enemy spawn location
    let currentEnemySpawn = 0;
    ...
    function onLoad(_event) {
        ...
        //The viewport
        Dreamers.viewport = new ƒ.Viewport();
        Dreamers.viewport.initialize("viewport", graph, camera, canvas);
        ...
        //The player
        player = new Dreamers.Player();
        enemyController = new Dreamers.EnemySpawnController();
        //The walls of the arena
        graph.addChild(new Dreamers.Map());
        //The update event runs every frame
        ƒ.Loop.addEventListener("loopFrame" /* LOOP_FRAME */, update);
        ƒ.Loop.start(ƒ.LOOP_MODE.TIME_REAL, 30);
        //Shooting/aiming Events
        Dreamers.viewport.activatePointerEvent("\u0192pointerdown" /* DOWN */, true);
        Dreamers.viewport.addEventListener("\u0192pointerdown" /* DOWN */, mouseDown);
        //Html button events
        let prevButton = document.getElementById("prev-wave");
        prevButton.addEventListener("click", previousWave);
        let nextButton = document.getElementById("next-wave");
    ...
    function update(_event) {
        ...
        if (player.update(camera)) {
            ...
            for (let i = 0; i < listOfEnemies.length; i++) {
                let enemy = listOfEnemies[i];
                enemy.mtxLocal.rotateY(1);
            }
            //Update the viewport
            Dreamers.viewport.draw();
            ...
            if (listOfBullets.length >= 1) {
                for (let i = 0; i < listOfBullets.length; i++) {
                    ...
                    for (let j = 0; j < listOfEnemies.length; j++) {
            ...
            if (enemiesRemaining > 0) {
                ...
                if (enemySpawnRecharge == 0) {
                    ...
                    let newEnemySpawn = Math.floor(Math.random() * 4) + 1;
                    ...
                    switch (enemyController.enemySpawnChoice(wave, difficulty)) {
                        case 1:
                            newEnemy = new Dreamers.SmallEnemy(currentEnemySpawn);
                            break;
                        case 2:
                            newEnemy = new Dreamers.MediumEnemy(currentEnemySpawn);
                            break;
                        case 3:
                            newEnemy = new Dreamers.BigEnemy(currentEnemySpawn);
                            break;
                        case 4:
                            newEnemy = new Dreamers.Boss(currentEnemySpawn);
                            break;
                    }
    ...
    function mouseDown(_event) {
        ...
        if (player.getHealth() >= 0.1) {
            ...
            let ray = Dreamers.viewport.getRayFromClient(new ƒ.Vector2(_event.pointerX, _event.pointerY));
            let pointOfIntersection = ray.intersectPlane(ƒ.Vector3.ZERO(), new ƒ.Vector3(0, 1, 0));
            ...
            let bullet = new Dreamers.Bullet(player.getComponent(ƒ.ComponentTransform).local.translation, pointOfIntersection);
